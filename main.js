...
//  win.executeJavaScript("document.getElementById('player_list')")
...
const cookie = {
  ...
  name: 'login',
  value: 'mai%252094143213109619111339746426584730573287',
  domain: 'crawl.kelbi.org',
  hostOnly: true,
  path: '/',
  secure: false,
  httpOnly: false,
  session: false,
  expirationDate: 1647713017,
...
app.whenReady().then(() => {
/*
  //session.defaultSession.cookies.set({ url: 'https://crawl.kelbi.org/', name: 'login', value: 'mai%2520132834577748990038691950270807491874591' })
  session.defaultSession.cookies.set({ url: 'https://crawl.kelbi.org/', name: 'login', value: 'mai%2013049953218235270891332773281215836497' }).then(() => {
    // success
  }, (error) => {
    console.error(error)
  })
  session.defaultSession.cookies.set({ url: 'https://crawl.kelbi.org/', name: 'aaaa', value: 'bbbb' })
...
/*
      ...
      let rows = document.querySelectorAll('#player_list tbody tr');
      for (let i of rows) {
        console.log('GAME: == IDLETIME: ', i.cells.8.innerText);
      };
